...
export function filterModels(probed, selectedIds) {
  ...
  return selectedIds.filter((id) => byId.has(id)).map((id) => byId.get(id));
}

/**
 * Read the model allowlist file (models.json). Returns [] when the file is
 * missing or malformed — i.e. allowlist disabled.
 * Schema: { "selected": ["auto", "claude-fable-5-medium", ...] }
 */
export function readSelectedModels(filePath) {
  try {
 ...
 */
export function listCursorModels({ cursorBin = "cursor-agent", timeoutMs = 15000, env = process.env } = {}) {
