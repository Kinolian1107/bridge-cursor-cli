...
// Run: node tests/probe-models.test.mjs
...
console.log("parseListModelsOutput");
...
console.log("readSelectedModels");

test("reads selected ids from models.json", () => {
  const dir = mkdtempSync(join(tmpdir(), "bridge-test-"));
  const file = join(dir, "models.json");
  writeFileSync(file, JSON.stringify({ selected: ["auto", "  composer-2.5  ", "", 42] }));
  assert.deepEqual(readSelectedModels(file), ["auto", "composer-2.5"]);
  rmSync(dir, { recursive: true, force: true });
});

test("missing or invalid file → empty allowlist", () => {
  assert.deepEqual(readSelectedModels("/nonexistent/models.json"), []);
  const dir = mkdtempSync(join(tmpdir(), "bridge-test-"));
  const file = join(dir, "models.json");
  writeFileSync(file, "not json");
  assert.deepEqual(readSelectedModels(file), []);
  rmSync(dir, { recursive: true, force: true });
});

console.log("cursor-cli helpers");
